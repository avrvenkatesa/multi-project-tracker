...
async function updateIssuesWithTimeline(projectId, timeline) {
  try {
    console.log(`📅 Updating issues with timeline data for project ${projectId}...`);
    ...
    if (timeline.phases && Array.isArray(timeline.phases)) {
      for (const phase of timeline.phases) {
        ...
        if (matchingIssue && phase.startDate && phase.endDate) {
          ...
          console.log(`✅ Updated issue "${matchingIssue.title}" with dates ${phase.startDate} to ${phase.endDate}`);
          updatedCount++;
        }
      }
    }
    ...
    if (timeline.milestones && Array.isArray(timeline.milestones)) {
      for (const milestone of timeline.milestones) {
        ...
        if (!exists && milestone.dueDate) {
          ...
          console.log(`✅ Created milestone issue: ${milestone.name} (ID: ${result.rows[0].id})`);
          createdCount++;
        }
      }
    }

    console.log(`📊 Timeline update complete: ${updatedCount} updated, ${createdCount} created`);
    return { updated: updatedCount, created: createdCount };

  } catch (error) {
    console.error('❌ Error updating issues with timeline:', error);
    console.error('Error details:', error.message);
    throw error;
  }
}
...
async function updateTaskDates(issueId, startDate, endDate) {
  try {
    ...
    console.log(`✅ Updated dates for issue ${issueId}: ${startDate} to ${endDate}`);
...
async function updateTaskProgress(issueId, progress) {
  try {
    ...
    console.log(`✅ Updated progress for issue ${issueId}: ${progress}% (status: ${status})`);
