...
class CSVExportService {
  ...
  async exportFullProject(projectId) {
    // Export issues and action items
    const issuesResult = await this.exportIssues(projectId);
    const actionItemsResult = await this.exportActionItems(projectId);
    
    // For now, return the issues file (could be expanded to ZIP multiple files)
    return issuesResult;
  }
}
