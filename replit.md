...
**PKG (Project Knowledge Graph) Overlay**: A unified graph abstraction layer built on PostgreSQL that provides a non-invasive overlay over all project entities. The PKG consists of two core tables: `pkg_nodes` (representing entities like Tasks, Risks, Decisions, Meetings) and `pkg_edges` (representing relationships like depends_on, parent_of, evidence_of, sourced_from). Each PKG node polymorphically references underlying source tables (issues, action_items, risks, decisions, meetings) through `source_table` + `source_id` fields, with denormalized JSONB `attrs` for fast querying. The PKG enables AI agents to query and manipulate entities through a single unified interface without rewriting existing code. Features include: UUID-based node IDs, AI provenance tracking (created_by_ai, ai_confidence), versioning support (superseded_by for decision history), JSONB attribute storage with GIN indexes, confidence scoring for AI-detected relationships, and evidence quote tracking. The overlay automatically seeds from existing data (currently 591 nodes across 41 projects, 235 hierarchical edges) and maintains bidirectional links via `pkg_node_id` in source tables. UNIQUE constraints on `pkg_nodes(source_table, source_id)` and `pkg_edges(project_id, type, from_node_id, to_node_id)` ensure one-to-one mapping and enable idempotent upserts via `INSERT ... ON CONFLICT DO NOTHING` for safe data synchronization. This architecture supports future AIPM capabilities including knowledge graph queries, relationship inference, and cross-entity analytics.

**RAG (Retrieval-Augmented Generation) Foundation**: A unified document indexing and semantic search system built on PostgreSQL full-text search. The RAG foundation consists of the `rag_documents` table with auto-generated `content_tsv` TSVECTOR columns for fast full-text search using GIN indexes. Documents are automatically indexed from meetings, decisions, and risks via database triggers that maintain real-time synchronization. The system supports: polymorphic source tracking (source_type + source_id), JSONB metadata storage, relevance-ranked search with `ts_rank()`, highlighted snippets via `ts_headline()`, and filtered queries by project/source type. API endpoints provide semantic search (`GET /api/aipm/projects/:projectId/rag/search`), manual document upload (`POST /api/aipm/projects/:projectId/rag/docs`), and LLM context assembly (`GET /api/aipm/rag/context`) with automatic token estimation and source citation. Auto-indexing triggers ensure all new meetings, decisions, and risks are immediately searchable. The architecture is designed for Phase 2 upgrade to pgvector for embedding-based semantic search while maintaining backward compatibility with current full-text search capabilities.
