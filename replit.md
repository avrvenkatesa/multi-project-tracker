...
### Frontend
The frontend is a single-page application (SPA) built with vanilla JavaScript and Tailwind CSS, featuring a dynamic UI based on user roles, real-time AI analysis capabilities, a comprehensive comment system, and a Project Dashboard with analytics and Chart.js visualizations. UI elements such as Kanban boards, tag displays, risk cards, and a comprehensive checklist system prioritize clarity and interactivity. The UI implements consistent navigation, blue header design with white text, and user information display with responsive design. All inline JavaScript is moved to external files for CSP compliance. Event delegation with data attributes (`data-action`, `data-checklist-id`, `data-template-id`) is used throughout to eliminate inline event handlers and ensure strict CSP compliance.

**Project Header Navigation**: The current project view displays four key navigation elements: View dropdown (purple, with Dashboard, Checklists, Templates, Tags, and Risks options), AI Analysis button (gradient purple-to-blue with ⚡ icon), Transcripts button (indigo with 🎙️ icon), and Create dropdown (green, with Issue and Action Item options). All buttons are CSP-compliant with proper event delegation.
...
- **Linked Tab**: Shows all checklists linked to issues or action items with completion percentages and related entities (CSP-compliant with `data-action="view-linked"` and `"delete-linked"`)
- **Standalone Tab**: Displays standalone checklists generated from documents with stats dashboard (total checklists, items, documents), search and sort functionality, and quick linking to issues/actions (CSP-compliant with `data-action="view-standalone"`, `"link-standalone"`, and `"delete-standalone"`)
- **Templates Tab**: Lists all available checklist templates for quick access and usage (CSP-compliant with `data-action="view-template"` and `"use-template"`)

The unified interface replaced the separate standalone-checklists.html page, improving UX by centralizing all checklist operations. Users can seamlessly switch between linked checklists, standalone checklists awaiting assignment, and templates. The "Checklist Library" option has been removed from View dropdowns across the application in favor of the Standalone tab. All tabs use event delegation with data attributes for zero CSP violations.
